...
class FileHandler:
    ...
    def saveWorld(world: World, name: str):
        ...
        with open(f"OOP-PG-2024/Project/Saves/{name}.json", "w") as file:
            json.dump(data, file, indent=4)

    @staticmethod
    def listSaves():
        return [os.path.splitext(f.name)[0] for f in sorted(list(filter(lambda f: f.is_file(), list(Path("OOP-PG-2024/Project/Saves/").glob("*.json")))))]

    @staticmethod
    def loadWorld(name: str) -> World:
        with open(f"OOP-PG-2024/Project/Saves/{name}.json", "r") as file:
            data = json.load(file)
